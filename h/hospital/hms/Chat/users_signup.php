...
$position = array("--Select Position--", "Director", "Managing Director", "Specialist", "GP");

$deparment = array("--Select Department--", "doctor", "nurse", "HO");
...
if (isset($_POST["submit"])) {
  ...
  if ($first_name != "" && $last_name != "" && $user_password != "" && $user_position != "" && $user_department != "") {
    ...
    $existing_user = "select * from users_table where user_fname = '$first_name'";
    $execute_existing_user = mysqli_query($connection_String, $existing_user);
    if (mysqli_num_rows($execute_existing_user) > 0) {
      echo "<script>alert('Sorry, You have already registered')</script>";
    } else {
      $register_user_command =
        "INSERT INTO users_table (`Users_ID`, `user_fname`, `user_lname`, `department`, `position`, `Password`) 
    VALUES (NULL, '$first_name', '$last_name', '$user_department', '$user_position', '$user_password')";
